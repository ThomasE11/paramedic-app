...
function TreatmentBayImmersionLayer({
  ...
  const chestStrap = treatmentBayClinicalToWorld([0, 1.26, 0.235]);
  const pelvisStrap = treatmentBayClinicalToWorld([0, 0.88, 0.235]);
  const thighStrap = treatmentBayClinicalToWorld([0, 0.52, 0.225]);
  const face = treatmentBayClinicalToWorld([0, 1.56, 0.26]);
  const chestLeft = treatmentBayClinicalToWorld([-0.10, 1.27, 0.25]);
  const chestRight = treatmentBayClinicalToWorld([0.11, 1.18, 0.25]);
  const ivSite = treatmentBayClinicalToWorld([-0.23, 0.82, 0.24]);
  ...
  return (
    <group>
      ...
      {[chestStrap, pelvisStrap, thighStrap].map((position, index) => (
        <mesh key={`stretcher-strap-${index}`} position={[position[0], position[1] + 0.035, position[2]]} raycast={() => null}>
          <boxGeometry args={[index === 0 ? 0.86 : 0.72, 0.032, 0.055]} />
          <meshStandardMaterial color={index === 2 ? '#182235' : '#0f172a'} roughness={0.52} metalness={0.12} transparent opacity={0.82} />
        </mesh>
      ))}

      <mesh position={[0, 0.62, 1.14]} raycast={() => null}>
        <boxGeometry args={[1.16, 0.13, 0.055]} />
        <meshStandardMaterial color="#020617" roughness={0.42} metalness={0.25} transparent opacity={0.42} />
      </mesh>
      <mesh position={[0, 0.705, 1.105]} raycast={() => null}>
        <boxGeometry args={[0.58, 0.022, 0.026]} />
        <meshStandardMaterial color="#facc15" roughness={0.45} metalness={0.08} transparent opacity={0.16} />
      </mesh>

      {[-0.58, 0.58].map(x => (
        <mesh key={`foreground-rail-${x}`} position={[x, 0.66, 0.36]} rotation={[Math.PI / 2, 0, 0]} raycast={() => null}>
          <cylinderGeometry args={[0.012, 0.012, 1.52, 14]} />
          <meshStandardMaterial color="#d6e4ee" roughness={0.28} metalness={0.66} transparent opacity={0.36} />
        </mesh>
      ))}
...
const TREATMENT_BAY_CAMERA_FOCUS = {
  pos: [1.05, 1.14, 1.32] as [number, number, number],
...
export function Body3DModel({ onRegionClick, assessedRegions, caseData, patientSounds, caseCategory, appliedTreatmentIds = [], patientVisualState = null, isInArrest = false, vitals, onPulse, treatmentBayMode = false, onOpenTreatments }: Body3DModelProps) {
  ...
  }, [onRegionClick, animateCamera, clearPatientReaction, anatomyLayer, caseData, patientVoice, treatmentBayOverviewEnabled]);
      ...
      <div className="patient-model-frame">
        <div className={`patient-model-canvas-shell relative min-w-0 overflow-hidden ${patientFirstExamLayout ? 'patient-first-canvas' : ''} ${activeRegion ? 'bg-slate-100/35 dark:bg-slate-950/20' : 'h-[320px] sm:h-[380px] lg:h-[420px]'}`}>
          <div className={`patient-model-canvas-stage relative min-w-0 overflow-hidden ${patientFirstExamLayout ? 'patient-first-canvas-stage' : activeRegion ? 'h-[430px] sm:h-[470px]' : 'h-full'}`}>
            <Canvas
              camera={{ position: overviewCameraFocus.pos, fov: useTreatmentBayPresentation ? 29 : 34 }}
