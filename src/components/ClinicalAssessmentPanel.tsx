...
function FindingBadge({ finding }: { finding: AssessmentFinding }) {
  ...
  return (
    <div className={`rounded-lg border p-2.5 ${colorMap[finding.severity]}`}>
      <div className="flex items-start gap-2">
        {finding.severity === 'critical' && <AlertTriangle className="h-3.5 w-3.5 shrink-0 mt-0.5" />}
        {finding.severity === 'abnormal' && <Info className="h-3.5 w-3.5 shrink-0 mt-0.5" />}
        {finding.severity === 'normal' && <CheckCircle2 className="h-3.5 w-3.5 shrink-0 mt-0.5" />}
        <div className="min-w-0">
          <span className="text-[11px] font-semibold uppercase tracking-wider opacity-70">{finding.label}</span>
          <p className="text-xs font-medium leading-relaxed">{finding.value}</p>
          {finding.significance && (
            <p className="text-[10px] mt-1 opacity-70 italic">{finding.significance}</p>
          )}
        </div>
      </div>
    </div>
...
function AssessmentStepButton({
  ...
  return (
    <Button
      ...
      onClick={onPerform}
      className={`
        justify-start gap-2 h-auto py-2.5 px-3 text-left w-full transition-all rounded-xl
        ${isPerformed
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          {isPerformed && performedOrder && (
            <Badge variant="outline" className="text-[9px] py-0 h-4 bg-green-500/10 border-green-500/30">
              #{performedOrder}
            </Badge>
          )}
          {!isPerformed && isRequired && (
            <Badge variant="outline" className="text-[9px] py-0 h-4 bg-white/20 border-white/30 text-white">
              Required
            </Badge>
          )}
        ...
        <span className="text-[10px] opacity-70 line-clamp-1">{step.description}</span>
      </div>
      {!isPerformed && (
...
function PhaseSection({
  ...
  return (
    <Card className={`border overflow-hidden rounded-2xl ${performedInPhase.length === totalInPhase && totalInPhase > 0 ? 'border-green-300 dark:border-green-800' : 'border-border/60'}`}>
      <CardHeader
        className={`pb-2 cursor-pointer hover:bg-muted/30 transition-colors ${phaseColor}`}
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500/15 to-blue-500/5">
            <Search className="h-4 w-4 text-blue-500" />
          </div>
          <div>
            <h3 className="text-sm font-bold tracking-tight">Clinical Assessment</h3>
            <p className="text-[11px] text-muted-foreground">
              Perform assessments to reveal findings — click each step
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant={overallPct === 100 ? 'default' : 'secondary'} className={`text-[10px] ${overallPct === 100 ? 'bg-green-500' : ''}`}>
