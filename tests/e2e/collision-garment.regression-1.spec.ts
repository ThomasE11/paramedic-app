...
for (const caseId of ['y1-011']) {
test(`collision trousers and lining follow the seated patient in ${caseId}`, async ({ page }, testInfo) => {
  ...
  await page.locator('.tactical-patient-viewport').screenshot({ path: testInfo.outputPath('collision-patient.png') });
  const measurements = await page.evaluate(() => {
    ...
    body.traverse(object => {
      if (!['scrub-trousers', 'scrub-trousers-lining'].includes(object.name)) return;
